...
export function buildAuditDoc(firmName: string, practiceArea: string, analysis: any, context: any, letter: any, submission: any): Document {
  ...
  if (portfolioCuration && (
    ...
    if (curation.officialPubMatters.length > 0) {
      const pubList = curation.officialPubMatters.map((m: any, i: number) => `#${String(i + 1).padStart(2, '0')} ${m.client || m.name || 'Matter'}${m.value ? ` (${m.value})` : ''}`).join(', ');
      dynamicRecCore.push(`PUBLISHABLE CORE (${curation.officialPubMatters.length} Matters — Final Matters Section D #01 to #${String(curation.officialPubMatters.length).padStart(2, '0')}): ${pubList}. Total: ${curation.officialPubMatters.length} Publishable Matters.`);
    }

    if (curation.officialConfMatters.length > 0) {
      const confList = curation.officialConfMatters.map((m: any, i: number) => `#${String(curation.officialPubMatters.length + i + 1).padStart(2, '0')} (Conf ${String(i + 1).padStart(2, '0')}) ${m.client || m.name || 'Matter'}${m.value ? ` (${m.value})` : ''}`).join(', ');
      dynamicRecCore.push(`CONFIDENTIAL CORE (${curation.officialConfMatters.length} Recommended Matters — Final Matters Section E #01 to #${String(curation.officialConfMatters.length).padStart(2, '0')}): ${confList}. Total: ${curation.officialConfMatters.length} Confidential Matters.`);
    }
    ...
    // Dynamic 1:1 Curation Reconciliation Table (All N Source Matters Tracked)
    if (availableMatters.length > 0) {
      sections.push(
        subTitle(`Matter-by-Matter Curation Audit Table (All ${availableMatters.length} Source Matters Tracked)`),
      ...
      const curationRows: string[][] = availableMatters.map((m: any, idx: number) => {
        ...
        const pubIdx = curation.officialPubMatters.findIndex((op: any) => op === m || op.id === m.id || (op.client && op.client === m.client && op.name === m.name));
        const confIdx = curation.officialConfMatters.findIndex((oc: any) => oc === m || oc.id === m.id || (oc.client && oc.client === m.client && oc.name === m.name));
      ...
      sections.push(makeTable(['Source Matter', 'Client', 'Decision', 'Final Position', 'Strategic Curation Rationale'], curationRows));
      sections.push(emptyRow());
    }

    // Dynamic Source Vulnerability Detection Engine
    const dynamicVulns: string[] = [];
    for (let i = 0; i < availableMatters.length; i++) {
      const m = availableMatters[i];
    ...
    const rawVulns = (Array.isArray(portfolioCuration?.source_vulnerabilities) && portfolioCuration.source_vulnerabilities.length > 0)
      ? portfolioCuration.source_vulnerabilities
      : (dynamicVulns.length > 0 ? dynamicVulns : [
          "Referee Availability: Ensure at least 2-3 responsive referee contacts with active email addresses are ready for directory interview outreach.",
          "Client Consent Verification: Verify all confidential matters have client-side clearance for directory researcher submission under rule Chatham House.",
          "Currency & Economic Scale: Verify that all financial figures reflect confirmed deal values or liability exposure under active management."
        ]);
