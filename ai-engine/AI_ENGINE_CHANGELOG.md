...
> Last updated: **2026-08-04** (v17.1 — Paradigm Shift: KEEP→EXPAND→STRENGTHEN matter enhancer, analysis_node prompt fix, gpt-4o JSON unwrapper, score extraction robustness, word count floor, entity-loss re-optimization)
...
| 85 | **Matter Enhancer (replaces Matter Optimizer)** — Paradigm: "You are a Matter Enhancer, NOT a Rewriter. You do NOT compress." Output must contain MORE facts than input, NEVER fewer. 100% fact preservation with programmatic validator (numbers, proper nouns, word ratio). Differentiation Rule: each matter must tell a unique story. Falls back to original text if compression detected | v16.0 | `prompts.py` (MATTER_ENHANCER_PROMPT), `utils/validators.py` (validate_matter_enhancement), `nodes.py` (optimization_node enhancement gate) | ✅ ACTIVE | 🔴 SUPREME |
...
| 95 | **Enhancement Gate Threshold Fix** — Proper noun threshold lowered from 80% to 50%. When matters are ENHANCED (expanded 3-5x from 50→200 words), new entities are added which dilutes the preservation ratio. Also filters out raw_text field labels (Title, Client, Value, Summary, Significance, Lead, Partner) from proper noun extraction since they inflate the expected count | v17.0 | `utils/validators.py` (validate_matter_enhancement proper_noun check) | ✅ ACTIVE | 🔴 CRITICAL |
...
| 98 | **Analysis Node 2-Call Decomposition** — Split analysis_node into 2 LLM calls. Call 1 generates the core audit letter (score, state_of_play, path_to_dominance, reality_check, competitive_context, closing). Call 2 generates matter_evaluations + recommended_rewrites. This prevents JSON truncation when gpt-4o output exceeds max_tokens=8192. GPT-4o API research confirmed that truncated JSON in json_object mode produces finish_reason="length" with invalid JSON. Each call now generates ~4K tokens instead of ~10K+ | v17.0 | `nodes.py` (analysis_node Call 2), `prompts.py` (MATTER_EVALUATIONS_PROMPT, schema split) | ✅ ACTIVE | 🔴 SUPREME |
| 99 | **Narrative Matter Extraction** — Extraction prompt now handles submissions where matters are embedded in narrative paragraphs (e.g., department description B7) instead of the standard D1-D20 / E1-E20 structured format. The LLM must identify each distinct client engagement, mandate, or advisory relationship mentioned in narrative text and extract it as a separate matter. Fixes AraqueReyna 0-matter bug where the Banking & Finance submission described all work in prose without structured matter tables | v17.0 | `prompts.py` (EXTRACTION_SYSTEM_PROMPT: NARRATIVE MATTER EXTRACTION section) | ✅ ACTIVE | 🔴 SUPREME |
...
| 103 | **Matter Enhancer Paradigm Shift: KEEP→EXPAND→STRENGTHEN** — Complete rewrite of MATTER_ENHANCER_PROMPT. New paradigm: "RankPilot does not summarize. It takes existing evidence and makes it MORE CONVINCING." Three Laws: (1) KEEP every fact, (2) EXPAND with editorial context 3-5x longer, (3) STRENGTHEN into Chambers-grade prose. Includes JP Morgan example showing wrong vs correct enhancement. Replaces v16.0 enhancer that still allowed summarization. TESTED: González matters expanded 13-27x, AraqueReyna matters expanded 15-31x | v17.1 | `prompts.py` (MATTER_ENHANCER_PROMPT) | ✅ ACTIVE | 🔴 SUPREME |
...
| 107 | **Word Count Floor Rule** — Mandatory minimum word count ratios for enhanced matters: ~50w input → 150-250w output (3-5x), ~100w → 200-350w (2-3.5x), ~200w → 250-400w (1.25-2x). Output MUST NEVER be shorter than input. If shorter, the enhancement FAILED | v17.1 | `prompts.py` (MATTER_ENHANCER_PROMPT: WORD COUNT RULE) | ✅ ACTIVE | 🔴 CRITICAL |
